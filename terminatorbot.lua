...
-- ⚙️ CONFIGURATION NOX TERMINATOR V170
local CFG = {
...
local title = Instance.new("TextLabel", f); title.Size = UDim2.new(1, 0, 0, 40); title.Text = "NOX TERMINATOR BOT"; title.TextColor3 = CFG.OFF_COLOR; title.TextSize = 16; title.Font = Enum.Font.GothamBold; title.BackgroundTransparency = 1
local status = Instance.new("TextLabel", f); status.Size = UDim2.new(1, 0, 0, 30); status.Position = UDim2.new(0, 0, 0, 35); status.BackgroundTransparency = 1; status.Font = Enum.Font.Code; status.TextSize = 18; status.RichText = true 
...
local pText = Instance.new("TextLabel", f); pText.Size = UDim2.new(1, 0, 0, 25); pText.Position = UDim2.new(0, 0, 0, 80); pText.Text = "SYSTEM STANDBY..."; pText.TextColor3 = CFG.OFF_COLOR; pText.TextSize = 13; pText.Font = Enum.Font.Code; pText.BackgroundTransparency = 1

-- 🛠️ V170 : Bouton mis à jour dynamiquement
local taskBtn = Instance.new("TextButton", f); taskBtn.Size = UDim2.new(1, -40, 0, 32); taskBtn.Position = UDim2.new(0, 20, 0, 110); taskBtn.Text = "⚔️ CIBLE : COMBAT [CAPS]"; taskBtn.TextColor3 = Color3.new(1,1,1); taskBtn.BackgroundColor3 = Color3.fromRGB(150, 0, 0); taskBtn.Font = Enum.Font.GothamBold; taskBtn.TextSize = 13; Instance.new("UICorner", taskBtn).CornerRadius = UDim.new(0, 4)
local predBtn = Instance.new("TextButton", f); predBtn.Size = UDim2.new(1, -40, 0, 32); predBtn.Position = UDim2.new(0, 20, 0, 150); predBtn.Text = "MOBILITÉ PREDATOR [OFF] (P)"; predBtn.TextColor3 = Color3.new(1,1,1); predBtn.BackgroundColor3 = Color3.fromRGB(30, 30, 30); predBtn.Font = Enum.Font.Code; predBtn.TextSize = 13; Instance.new("UICorner", predBtn).CornerRadius = UDim.new(0, 4)

local vipLabel = Instance.new("TextLabel", f); vipLabel.Size = UDim2.new(1, 0, 0, 25); vipLabel.Position = UDim2.new(0, 0, 0, 190); vipLabel.BackgroundTransparency = 1; vipLabel.Font = Enum.Font.GothamBold; vipLabel.TextSize = 12; vipLabel.Text = "👑 COURONNE : NON DÉTECTÉE"; vipLabel.TextColor3 = Color3.fromRGB(100, 100, 100)

local infoBox = Instance.new("TextLabel", f); infoBox.Size = UDim2.new(1, -20, 0, 45); infoBox.Position = UDim2.new(0, 10, 0, 220); infoBox.BackgroundTransparency = 1; infoBox.TextColor3 = Color3.fromRGB(180, 180, 180); infoBox.TextSize = 11; infoBox.Font = Enum.Font.Code; infoBox.Text = "[ALT] POWER | [MOLETTE] PVP/PVE\n[CAPS] MODE | [P] PREDATOR | [J] MASQUER UI"; infoBox.TextWrapped = true
...
local function updateStatusUI()
    ...
    status.Text = string.format("<b><font color='%s'>%s</font></b> | <b><font color='%s'>%s</font></b>", sCol, botActive and "ONLINE" or "OFFLINE", mCol, currentMode)
end
updateStatusUI()
...
local function toggleTask()
    ...
    -- 🛠️ V170 : Bouton avec le bon nom !
    taskBtn.Text = currentTask == "COMBAT" and "⚔️ CIBLE : COMBAT [CAPS]" or "💰 CIBLE : LOOT ET COURONNE [CAPS]"
...
UIS.InputBegan:Connect(function(input, gpe)
    ...
    elseif input.KeyCode == MASTER_KEY then botActive = not botActive; StopAllInputs(); if not botActive then pText.Text = "SYSTEM STANDBY..."; pBar.BackgroundColor3 = CFG.OFF_COLOR end; updateStatusUI()
    elseif input.KeyCode == TASK_KEY then toggleTask() 
    elseif input.KeyCode == PHASE_KEY then executePhaseStep(CFG.PHASE_DISTANCE)
    elseif input.KeyCode == PREDATOR_KEY then predatorMode = not predatorMode; predBtn.Text = predatorMode and "⚡ PREDATOR [ON] ⚡" or "MOBILITÉ PREDATOR [OFF] (P)"; predBtn.BackgroundColor3 = predatorMode and Color3.fromRGB(150, 0, 0) or Color3.fromRGB(30, 30, 30)
...
RunService.RenderStepped:Connect(function()
    ...
    if t - vipCheckTimer > 1.0 then
        ...
        if vipHrp and vipHrp.Parent then
            ...
            vipLabel.Text = "👑 COURONNE DÉTECTÉE : " .. string.upper(pName)
            vipLabel.TextColor3 = Color3.new(1, 0.84, 0)
        else
            vipLabel.Text = "👑 COURONNE : NON DÉTECTÉE"
    ...
    if canPhase and (t - lastStuckCheck) > 0.75 then
        if (KeysHeld[KEY_Z] or KeysHeld[KEY_S] or KeysHeld[KEY_Q] or KeysHeld[KEY_D]) then 
            if (root.Position - lastPosRecord).Magnitude < 2.5 then 
                ...
                pText.Text = "⚠️ GHOST-PHASE (WALLHACK)..."; pBar.BackgroundColor3 = CFG.PURPLE_PHASE
            end
    ...
    if currentTask == "COMBAT" then
        ...
        if not target then StopMovementInputs(); pText.Text = "🤖 RECHERCHE D'ENNEMIS..."; pBar.BackgroundColor3 = CFG.NEON_CORE
        else
            local isRed = table.find(nemesysPlayers, target.Parent)
            local isP = Players:GetPlayerFromCharacter(target.Parent)
            
            if isRed and isP then pText.Text = "⚠️ KILLING RED PLAYER !"; pBar.BackgroundColor3 = CFG.PURPLE_PHASE
            elseif isRed then pText.Text = "⚠️ KILLING RED MOB !"; pBar.BackgroundColor3 = Color3.new(1, 0.5, 0)
            else pText.Text = "⚔️ FOCUSED EXTERMINATION..."; pBar.BackgroundColor3 = Color3.new(1, 0, 0) end
        ...
        if not loot then 
            ...
            if crownHolder then
                ...
                if dH > CFG.FOLLOW_DIST then
                    ...
                    pText.Text = "👑 VIP LEECH ACTIVE..."; pBar.BackgroundColor3 = Color3.new(1, 0.84, 0)
                    if math.random(1, 25) == 1 then hum.Jump = true end
                else
                    StopMovementInputs()
                    camera.CFrame = camera.CFrame:Lerp(CFrame.lookAt(camera.CFrame.Position, hPos), 0.15)
                    pText.Text = "🛡️ PROTECTING VIP."; pBar.BackgroundColor3 = Color3.new(1, 0.84, 0)
                end
            else
                -- 🛠️ V170 : UI Update "RECHERCHE LOOT ET COURONNE"
                isSecuringLoot = false; SetKey(INTERACT_KEY, false); StopMovementInputs(); pText.Text = "🤖 RECHERCHE LOOT ET COURONNE..."; pBar.BackgroundColor3 = Color3.new(0, 0.8, 0)
            end
        else
            ...
            if dL > actDist then 
                ...
                StopMovementInputs(); hum.WalkSpeed = 0; camera.CFrame = CFrame.lookAt(camera.CFrame.Position, lootPos); pText.Text = "💰 SECURING CHEST..."; pBar.BackgroundColor3 = Color3.new(1, 0.8, 0)
